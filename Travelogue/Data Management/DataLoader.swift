    ...
    func loadJson() {
        ...
        do {
            ...
            for entry in entries {
                ...
                if realm.objects(Region.self).filter(NSPredicate(format: "name = %@", entry.region)).count == 0 {
                    let region = Region()
                    region.name = entry.region
                    realm.add(region)
                }
                // Create country if not created
                if realm.objects(Country.self).filter(NSPredicate(format: "name = %@", entry.country)).count == 0 {
                    let country = Country()
                    country.name = entry.country
                    realm.add(country)
                }
                // Create place if not created
                if realm.objects(Place.self).filter(NSPredicate(format: "name = %@", entry.place)).count == 0 {
                    let place = Place()
